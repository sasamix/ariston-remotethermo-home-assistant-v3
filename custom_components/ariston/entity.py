...
class AristonEntity(CoordinatorEntity, ABC):
    ...
    def unique_id(self):
        ...
        # These two controls use the original single-zone registry IDs. Other
        # zoned Number entities include the zone in the name and again here.
        if (
            self.zone
            and getattr(self.entity_description, "zone", False)
            and self.entity_description.key not in (
                ThermostatProperties.HEATING_FLOW_TEMP,
                ThermostatProperties.HEATING_FLOW_OFFSET,
            )
        ):
